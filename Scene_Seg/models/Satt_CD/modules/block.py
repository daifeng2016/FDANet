...
from models.utils import unetConv2,unetConv2_res,unetConv2R
...
class GAU(nn.Module):
    def __init__(self, channels_high, channels_low, upsample=True,use_res=False):
        ...
        if use_res:
            ...
            #self.conv_last = unetConv2R(channels_low,channels_low)
        else:
...
class GAU_Res(nn.Module):
    def __init__(self, channels_high, channels_low, upsample=True):
        super(GAU_Res, self).__init__()
        # Global Attention Upsample
        self.upsample = upsample
        # self.conv3x3 = nn.Conv2d(channels_low, channels_low, kernel_size=3, padding=1, bias=False)
        # self.bn_low = nn.BatchNorm2d(channels_low)
        ...
        self.conv_bn_low=nn.Sequential(
            nn.Conv2d(channels_low, channels_low, kernel_size=3, padding=1),
            nn.BatchNorm2d(channels_low),
            nn.ReLU(inplace=True)
        )
        ...
        #self.relu = nn.ReLU(inplace=True)
        self.att=ChannelAttentionHL(channels_high,channels_low)
        self.conv_cat=ResBlock(channels_low,channels_low)
    ...
    def forward(self, fms_high, fms_low, fm_mask=None):
        ...
        # fms_low_mask = self.conv3x3(fms_low)#[2,1024,16,16]
        # fms_low_mask = self.bn_low(fms_low_mask)
        fms_high_out=self.conv_upsample(fms_high)
        fms_low=self.conv_bn_low(fms_low)

        fms_att = fms_low* fms_high_gp
        #out=self.conv_cat(torch.cat([fms_high_out,fms_att], 1))
        out=self.conv_cat(fms_high_out+fms_att)



        return out
class GAU_Res0(nn.Module):
    def __init__(self, channels_high, channels_low, upsample=True):
        super(GAU_Res0, self).__init__()
        # Global Attention Upsample
        self.upsample = upsample
        # self.conv3x3 = nn.Conv2d(channels_low, channels_low, kernel_size=3, padding=1, bias=False)
        # self.bn_low = nn.BatchNorm2d(channels_low)
        ...
        #self.relu = nn.ReLU(inplace=True)
        self.att=ChannelAttentionHL(channels_high,channels_low)
        self.conv_cat=ResBlock(channels_low*2,channels_low)
...
class GAU_Res1(nn.Module):
    def __init__(self, channels_high, channels_low, upsample=True):
        super(GAU_Res1, self).__init__()
        # Global Attention Upsample
        self.upsample = upsample
        # self.conv3x3 = nn.Conv2d(channels_low, channels_low, kernel_size=3, padding=1, bias=False)
        # self.bn_low = nn.BatchNorm2d(channels_low)
...
class GAU_Res2(nn.Module):
    def __init__(self, channels_high, channels_low, upsample=True):
        super(GAU_Res2, self).__init__()
        # Global Attention Upsample
        self.upsample = upsample
        # self.conv3x3 = nn.Conv2d(channels_low, channels_low, kernel_size=3, padding=1, bias=False)
        # self.bn_low = nn.BatchNorm2d(channels_low)
