...
#=========================cam refine module======================================
#ref:Self-supervised Equivariant Attention Mechanism for Weakly Supervised Semantic Segmentation
    # def PCM(self, cam, f):
    #     n, c, h, w = f.size()
    #     cam = F.interpolate(cam, (h, w), mode='bilinear', align_corners=True).view(n, -1, h * w)
    #     f = self.f9(f)  # self.f9 = torch.nn.Conv2d(192+3, 192, 1, bias=False)
    #     f = f.view(n, -1, h * w)
    ...
    #     return cam_rv
    '''
    We use ReLU activation function with L1 normalization to mask out irrelevant pixels and
generate an affinity attention map which is smoother in relevant regions
    '''
...
if __name__ == '__main__':
    ...
    pred=torch.randn(size=(1, 3, 128, 128))
