...
class _GridAttentionBlockND(nn.Module):
    ...
    def _concatenation(self, x, g):
        input_size = x.size()# x [1,128,16,16,5] g [1,128,8,8,5]
        batch_size = input_size[0]# 1
        assert batch_size == g.size(0)

        # theta => (b, c, t, h, w) -> (b, i_c, t, h, w) -> (b, i_c, thw)
        # phi   => (b, g_d) -> (b, i_c)
        theta_x = self.theta(x)#[1,128,16,16,5]==>[1,128,8,8,2]
        theta_x_size = theta_x.size()

        # g (b, c, t', h', w') -> phi_g (b, i_c, t', h', w')
        #  Relu(theta_x + phi_g + bias) -> f = (b, i_c, thw) -> (b, i_c, t/s1, h/s2, w/s3)
        phi_g = F.upsample(self.phi(g), size=theta_x_size[2:], mode=self.upsample_mode)#[1,128,8,8,5]==>[1,128,8,8,2]
        f = F.relu(theta_x + phi_g, inplace=True)#[1,128,8,8,2]

        #  psi^T * f -> (b, psi_i_c, t/s1, h/s2, w/s3)
        sigm_psi_f = F.sigmoid(self.psi(f))#[1,128,8,8,2]==>[1,1,8,8,2]

        # upsample the attentions and multiply
        sigm_psi_f = F.upsample(sigm_psi_f, size=input_size[2:], mode=self.upsample_mode)#[1,1,8,8,2]==>[1,1,16,16,5]
        y = sigm_psi_f.expand_as(x) * x#expand_as把一个tensor变成和函数括号内一样形状的tensor  [1,1,16,16,5]==>[1,128,16,16,5]
        W_y = self.W(y)#[1,128,16,16,5]==>[1,128,16,16,5]
