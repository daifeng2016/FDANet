...
import losses.loss_utils as UF


class SparseCRFLoss(nn.Module):


    def __init__(self,  sigma=0.15, subtract_eps=0.0,num_classes=2, negative_class=False, diag=False):
    ...
    def forward(self, y_pr, y_gt, img):
        ...
        for ch in range(1, self.num_classes):
            loss = loss + UF.regularized_loss_per_channel2(mask_h, mask_v, ch, y_pr)
